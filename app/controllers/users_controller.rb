class UsersController < ApplicationController
  ...
  # POST /users.json
  def create
    params[:user][:rol_id] = 3
    @user = User.new(user_params)

    respond_to do |format|
      if @user.save
        Mailer.create_user(@user).deliver
        format.html { redirect_to root_path , notice: "El usuario #{@user.username} fue creado exitosamente." }
  ...
  private
    ...
    def user_params
      params.require(:user).permit(:username, :password, :password_confirmation, :name, :lastname, :email, :rol_id)
    end
end
